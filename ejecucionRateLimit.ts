...
async function ejecutar(): Promise<void> {
  ...
  process.loadEnvFile(`.env.${args.env}`);
  ...
  const reportFile     = `reports/K6-report/rateLimit_dashboard_${reportLabel}${scenarioSuffix}_${fecha}.html`;
  ...
  const command = [
    ...
    ...k6EnvVars,
    'src/test/performance/rateLimitTest.ts',
  ].join(' ');
  ...
  try {
    ...
    await Logs.agregarLineaAlLogHeader('Ejecución de rate limit completada exitosamente.');
  } catch (error: unknown) {
